...
#addprocs(8) # uncomment to add more processes
...
function simulation(n,N,path) 
    ...
    @showprogress @distributed for i in 1:N 
        S[i] = entropy(heightsDyckPath(randomDyckBridge(n))) # Dyck excursions
        #S[i] = entropy(heightsDyckPath(randomDyckBridge(n))) # Dyck bridges
    end 
    println("Done") 
    output=replace(string( path,"entropy_exc_Npts",n,"_Nsim",N,".csv" )," "=>"") # Dyck excursions
    # output=replace(string( path,"entropy_brd_Npts",n,"_Nsim",N,".csv" )," "=>"") # Dyck bridges
